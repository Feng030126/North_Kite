...
class AddPostFragment : Fragment() {
                ...
                imageRef.putFile(it)
                    .addOnSuccessListener { taskSnapshot ->
                        imageRef.downloadUrl.addOnSuccessListener { uri ->
                            currentUser.profilePictureUrl?.let { it1 ->
                                savePost(currentUser.username,
                                    it1, description, uri.toString())
                            }
                        }
                    }
    ...
    private fun savePost(username: String, profilePicUrl: String, description: String, imageUrl: String) {
        ...
        val post = Post(username, profilePicUrl, imageUrl, 0, description)
        if (postId != null) {
